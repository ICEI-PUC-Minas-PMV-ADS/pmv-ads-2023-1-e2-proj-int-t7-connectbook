...
|RF06   | O sistema deve comunicar ao usuário do tipo doador que um de seus livros para doação foi solicitado e iniciar o processo de doação entre receptor/doador via comunicação utilizando de plataformas de email |ALTA|
|RF07   | O sistema deve permitir a emissão de relatórios -- |MÉDIA|
